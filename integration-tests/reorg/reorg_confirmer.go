...
func (rc *ReorgController) isAltBlock(header blockchain.NodeHeader) bool {
...
func (rc *ReorgController) compareBlocks(header blockchain.NodeHeader) error {
	if header.NodeID == 0 && header.Number.Int64() >= rc.forkBlockNumber && rc.isAltBlock(header) {
		rc.once.Do(func() {
			log.Warn().Int64("Number", header.Number.Int64()).Msg("Reorg started")
			rc.reorgStarted <- struct{}{}
		})
		rc.altBlockNumbers = append(rc.altBlockNumbers, header.Number.Int64())
		rc.currentAltBlocks++
		log.Info().
			Int64("Number", header.Number.Int64()).
			Str("Hash", header.Hash().String()).
			Int("Node", header.NodeID).
