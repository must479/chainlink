...
func TestIntegration_KeeperPluginForwarderEnabled(t *testing.T) {
	...
	bootstrapNodePort := int64(19599)
	appBootstrap, bootstrapPeerID, bootstrapTransmitter, bootstrapKb := setupNode(t, bootstrapNodePort, "bootstrap_keeper_ocr", nodeKeys[0], backend, nil)
	// _, _, _, err = authorized_forwarder.DeployAuthorizedForwarder(genesisData[nodeKeys[0].Address], backend, common.Address{}, owner.From, common.Address{}, []byte{})
	// bootstrapForwarder := setupForwarderForNode(t, appBootstrap, sergey, nodeKeys[0].Address, backend, nodeKeys[0].Address, linkAddr)
	bootstrapNode := Node{
		appBootstrap, bootstrapTransmitter, bootstrapKb,
	}
	var (
